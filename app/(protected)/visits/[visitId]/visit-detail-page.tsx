...
function getErrorMessage(error: unknown, fallback: string): string {
  ...
  function TimelineEventIcon({ eventType }: { readonly eventType: string }) {
    if (eventType === "ARRIVAL_REGISTERED") return <LoginIcon className="!text-lg" />;
  ...
  function useVisitDetailPageModel(visitId: string): VisitDetailViewModel {
    ...
    const [keycardState, setKeycardState] = useState<RequestState<KeycardResponse>>(
    ...
    const reversedTimeline = useMemo(() => [...sortedTimeline].reverse(), [sortedTimeline]);
    ...
    const keycardNumber = detail?.keycardNumber ?? keycardState.data?.keycardNumber ?? null;
    ...
    async function handleRegisterDeparture() {
      ...
      try {
        ...
        setActionError(getErrorMessage(error, "Lahkumise registreerimine ebaõnnestus."));
      } finally {
  ...
  function EditVisitModal({ visitId, detail, onClose, onSuccess }: EditVisitModalProps) {
    const [accessPoints, setAccessPoints] = useState<AccessPointResponse[]>([]);
    const [accessPointId, setAccessPointId] = useState(detail?.accessPointId ?? "");
    const [entryTime, setEntryTime] = useState(toDatetimeLocal(detail?.arrivalTime));
    ...
    const [assignorResults, setAssignorResults] = useState<PersonInRoleResponse[]>([]);
    const [isSearchingAssignor, setIsSearchingAssignor] = useState(false);
    const [selectedAssignor, setSelectedAssignor] = useState<PersonInRoleResponse | null>(null);
    ...
    async function handleAssignorSearch() {
      ...
      try {
        const results = await searchEmployees(assignorQuery.trim(), assignorAbortRef.current.signal);
    ...
    async function handleSubmit(e: React.FormEvent) {
      ...
      const body: EditVisitRequest = {
        ...
        entryTime: entryTime
          ...
          : detail?.arrivalTime
            ? detail.arrivalTime
            : undefined,
        comment: comment.trim() !== "" ? comment.trim() : detail?.visitReason || undefined,
      ...
      try {
        ...
        setSubmitError(getErrorMessage(err, "Külastuse muutmine ebaõnnestus. Proovi uuesti."));
      } finally {
          ...
          <div className="flex-1 overflow-y-auto space-y-6 p-7">
            ...
            <ModalField label="Võõrustaja">
              {detail?.hostName ? (
                <p className="mb-2 text-xs text-slate-400">
                  Võõrustaja:{" "}
                  <span className="font-semibold text-slate-600">{detail.hostName}</span>
              ...
              <PersonSearchField
                ...
                onQueryChange={(v) => { setHostQuery(v); setHostResults([]); }}
                onSearch={handleHostSearch}
                onSelect={(p) => { setSelectedHost(p); setHostResults([]); setHostQuery(""); }}
                onClear={() => { setSelectedHost(null); setHostQuery(""); setHostResults([]); }}
            ...
            <ModalField label="Muudatuse tegija">
              <PersonSearchField
                ...
                placeholder="Otsi töötajat nimega…"
                onQueryChange={(v) => { setAssignorQuery(v); setAssignorResults([]); }}
                onSearch={handleAssignorSearch}
                onSelect={(p) => { setSelectedAssignor(p); setAssignorResults([]); setAssignorQuery(""); }}
                onClear={() => { setSelectedAssignor(null); setAssignorQuery(""); setAssignorResults([]); }}
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[230px_minmax(0,1fr)] gap-8 items-start">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
              ...
              />
              <InfoField
                label={arrivalIsFuture ? "Oodatav saabumise aeg" : "Saabumise aeg"}
                value={formatDateTime(arrivalTime)}
                icon={<AccessTimeIcon className="!text-base" />}
                future={arrivalIsFuture}
              />
              <InfoField
                label={departureIsFuture ? "Oodatav lahkumise aeg" : "Lahkumise aeg"}
  ...
  function VisitAuditLogSection({
    ...
    const isLoadingInitialTimeline = timelineState.isLoading && !timelineState.data;
    const timelineError = timelineState.error;
    const hasTimelineError = timelineError != null;
    const shouldShowEmpty =
      !timelineState.isLoading && !hasTimelineError && reversedTimeline.length === 0;
  ...
  function VisitKeycardSection({
    ...
    return (
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm p-8">
        ...
        {hasLinkedCard && keycardState.isLoading ? <SectionSkeleton lines={2} /> : null}
        ...
        {hasLinkedCard && hasKeycardData ? (
          <div className="space-y-5">
            <MetaRow label="Kaardi number" value={formatFieldValue(keycardData.keycardNumber)} />
            <MetaRow label="Staatus" value={formatFieldValue(keycardData.status)} />
            <MetaRow label="Määratud kasutaja" value={formatFieldValue(keycardData.assignedUser)} />
            <MetaRow label="Viimati tagastatud" value={formatDateTime(keycardData.lastReturnTime)} />
          </div>
        ) : null}
      </section>
    );
  ...
  export function VisitDetailPage({ visitId }: VisitDetailPageProps) {
    ...
    if (model.detailState.isLoading && !model.detail) return <VisitDetailSkeleton />;
  ...
  function InfoField({
    ...
    return (
      <div className="space-y-2">
        ...
        >
          <span className={future ? "text-sky-400" : "text-slate-300"}>{icon}</span>
          {label}
        </div>
        <div
          className={cn(
            "text-slate-900",
            prominent ? "text-3xl font-black tracking-tight" : "text-xl font-semibold",
